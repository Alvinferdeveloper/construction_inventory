...
import { LogIn, PackagePlus, PackageSearch, Warehouse } from "lucide-react";
import EntradaFormModal from "./entrada-form-modal";
import AddMaterialFormModal from "./add-material-form-modal";
import { getUnassignedMaterials } from "@/app/lib/queries/materiales";
...
export default async function BodegueroInventoryView({ userId }: BodegueroInventoryViewProps) {
    ...
    return (
        <div className="space-y-8">
            {await Promise.all(bodegas.map(async (bodega) => {
                const unassignedMaterials = await getUnassignedMaterials(bodega.id);
                return (
                    <Card key={bodega.id}>
                        <CardHeader className="flex flex-row items-center justify-between">
                            ...
                            <AddMaterialFormModal bodegaId={bodega.id} unassignedMaterials={unassignedMaterials}>
