import { FileText, Layers, History, Users, Warehouse, Package, FileCheck, ClipboardList, FolderGit } from "lucide-react";
...
import SupervisorProjectReport from "./components/SupervisorProjectReport";
import SupervisorHistoryReport from "./components/SupervisorHistoryReport"; // Import this

export default async function ReportsPage({ searchParams }: { searchParams: Promise<Record<string, string | string[] | undefined>> }) { // Corrected searchParams type
    ...
    // BODEGUERO VIEW
    if (user.rol === ROLES.BODEGUERO) { // Corrected role check
        return (
            <div className="min-h-screen bg-background p-8">
                <div className="max-w-7xl mx-auto">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-8">
                        <Link href="/reportes?report=my_movements" className={`block p-6 rounded-lg border-2 transition-colors ${!activeReport || activeReport === 'my_movements' ? 'border-primary bg-primary/5' : 'bg-card hover:border-muted-foreground/50'}`}>
                    ...
                    {(!activeReport || activeReport === 'my_movements') && (
                ...
                <div className="max-w-7xl mx-auto">
                    <div className="flex items-center justify-between mb-8">
                        <div className="flex items-center gap-3">
                            ...
                            <div>
                                <h1 className="text-3xl font-bold text-foreground">Mis Reportes</h1>
                                <p className="text-muted-foreground text-sm">Informes sobre tus proyectos y solicitudes.</p>
                            </div>
                        </div>
                    </div>

                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-8">
                        <Link href="/reportes?report=project_usage" className={`block p-6 rounded-lg border-2 transition-colors ${!activeReport || activeReport === 'project_usage' ? 'border-primary bg-primary/5' : 'bg-card hover:border-muted-foreground/50'}`}>
                            <div className="flex items-center gap-4">
                                <FolderGit className="w-8 h-8 text-primary" />
                                <div>
                                    <h3 className="text-lg font-semibold text-foreground">Uso de Materiales por Proyecto</h3>
                                    <p className="text-sm text-muted-foreground">Consumo total de materiales por proyecto.</p>
                                </div>
                            </div>
                        </Link>
                        <Link href="/reportes?report=received_history" className={`block p-6 rounded-lg border-2 transition-colors ${activeReport === 'received_history' ? 'border-primary bg-primary/5' : 'bg-card hover:border-muted-foreground/50'}`}>
                            <div className="flex items-center gap-4">
                                <History className="w-8 h-8 text-primary" />
                                <div>
                                    <h3 className="text-lg font-semibold text-foreground">Historial de Recibidos</h3>
                                    <p className="text-sm text-muted-foreground">Historial filtrable de materiales recibidos.</p>
                                </div>
                            </div>
                        </Link>
                    </div>

                    {(!activeReport || activeReport === 'project_usage') && (
                        <Suspense fallback={<TableSkeleton columns={3} />}>
                            <SupervisorProjectReport userId={user.id} />
                        </Suspense>
                    )}
                    {activeReport === 'received_history' && (
                        <Suspense fallback={<TableSkeleton columns={5} />}>
                            <SupervisorHistoryReport userId={user.id} searchParams={await searchParams} />
                        </Suspense>
                    )}
                </div>
            </div>
