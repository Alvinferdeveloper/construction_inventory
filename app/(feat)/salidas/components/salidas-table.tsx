...
import { type getSalidas } from "@/app/lib/queries/salidas";

type SalidasPromise = ReturnType<typeof getSalidas>;
type Salidas = SalidasPromise extends Promise<{ salidas: infer T }> ? T : never;
    ...
    if (salidas.length === 0) {
        return (
            <Card className="flex flex-col items-center justify-center h-96 border-dashed">
                ...
                <p className="text-muted-foreground">Aún no se ha registrado ninguna salida de material.</p>
            </Card>
        );
    }
